...
export default function EditAdminResourcePage() {
  ...
  const { control, register, handleSubmit, formState: { errors }, setValue, watch } = useForm<ResourceFormValues>({
    ...
    if (resourceId) {
      ...
      if (storedResources) {
        ...
        if (resourceToEdit) {
          ...
          setValue('subjectOrCourse', resourceToEdit.subjectOrCourse);
          setValue('isPremium', resourceToEdit.isPremium);
          setValue('imageUrl', resourceToEdit.imageUrl || '');
          setValue('contentUrl', resourceToEdit.contentUrl || '');
        } else {
          setItemNotFound(true);
          toast({
            title: "Error",
            description: "Resource not found in storage.",
            variant: "destructive",
          });
        }
      } else {
        setItemNotFound(true);
         toast({
            title: "Error",
            description: "No resource data found in storage.",
            variant: "destructive",
          });
      }
    }
  }, [resourceId, setValue, toast]);
  ...
  const onSubmit = async (data: ResourceFormValues) => {
    ...
    try {
      ...
      const resourceIndex = resources.findIndex(res => res.id === resourceId);
      if (resourceIndex > -1) {
        resources[resourceIndex] = { 
            ...resources[resourceIndex], 
            ...data,
            imageUrl: data.imageUrl || 'https://placehold.co/600x400.png',
            dataAiHint: data.type, // Update AI hint if type changes
        };
        ...
        router.push('/dashboard/admin/resources');
         ...
         toast({
          title: "Error",
          description: "Could not find resource to update.",
          variant: "destructive",
        });
        setItemNotFound(true);
      }
      ...
      <Card className="shadow-lg max-w-md mx-auto">
        <CardHeader>
          <CardTitle className="text-2xl flex items-center text-destructive">
            <AlertTriangle className="mr-2 h-6 w-6" /> Resource Not Found
          </CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-muted-foreground">The resource you are trying to edit does not exist or could not be loaded.</p>
          <Button onClick={() => router.push('/dashboard/admin/resources')} className="mt-4">
  ...
  return (
    <Card className="shadow-lg max-w-2xl mx-auto">
      <CardHeader>
        ...
        <CardDescription>
          Modify the details for the study resource.
        </CardDescription>
      </CardHeader>
      <CardContent>
